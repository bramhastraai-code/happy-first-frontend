...
interface ChatSharedMediaPanelProps {
  open: boolean;
  communityId: string;
  onClose: () => void;
...
export function ChatSharedMediaPanel({
  ...
  onClose,
  onJumpToMessage,
}: ChatSharedMediaPanelProps) {
  const [tab, setTab] = useState<(typeof TABS)[number]['id']>('image');

  const mediaQuery = useInfiniteQuery({
    queryKey: ['community-shared-media', communityId, tab],
    enabled: open,
    initialPageParam: 1,
    queryFn: async ({ pageParam }) => {
      const res = await communityAPI.sharedMedia(communityId, {
        type: tab,
        page: pageParam,
        limit: 24,
      });
              ...
              {items.map((item) => (
                ...
                >
                  <span className="mt-0.5 inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary">
                    {tab === 'link' ? <Link2 className="h-4 w-4" /> : <FileText className="h-4 w-4" />}
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-medium text-[#111b21]">
                      {item.fileName || item.url || item.text || 'Item'}
                    </span>
