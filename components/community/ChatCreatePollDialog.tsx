...
interface ChatCreatePollDialogProps {
  open: boolean;
  communityId: string;
  onClose: () => void;
  onCreated: (message: Awaited<ReturnType<typeof communityAPI.createPoll>>['data']['data']['message']) => void;
}
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      if (message.messageType !== 'poll' || !message.poll) {
        throw new Error(
          'Server did not create a poll (backend may need a restart). Stop npm run start in the backend terminal, start it again, then retry.'
        );
      }
      return message;
    },
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl bg-white p-4 shadow-xl">
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            className={cn(
              'inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground',
              'disabled:opacity-50'
            )}
          >
