...
export function ProfilePostViewer({
  ...
  const editMentionPeopleQuery = useQuery({
    ...
    enabled: Boolean(editPost) && editMentionQuery !== null,
    queryFn: async () => {
  ...
  const editTrendingHashtagsQuery = useQuery({
    ...
    enabled: Boolean(editPost) && editHashtagQuery === '',
    queryFn: async () => {
  ...
  const editHashtagSearchQuery = useQuery({
    ...
    enabled: Boolean(editPost) && editHashtagQuery !== null && editHashtagDebounced.length > 0,
    queryFn: async () => {
