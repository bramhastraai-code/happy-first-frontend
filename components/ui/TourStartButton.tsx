...
interface TourStartButtonProps {
  onClick: () => void;
  className?: string;
  /** When true, omit fixed positioning (for use inside PageFabColumn). */
  inline?: boolean;
}

export default function TourStartButton({ onClick, className, inline = false }: TourStartButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
